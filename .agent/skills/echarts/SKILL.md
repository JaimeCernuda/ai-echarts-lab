...
## 1. Theme Awareness
Always initialize charts using the custom Falcon theme logic provided in the global scope.
- **Pattern**: `echarts.init(dom, theme)`
- **Themes**: `falconLight` and `falconDark` are available as global window objects.
- **Implementation**:
  ```javascript
  // Default to what matches the system if not explicitly toggled, 
  // but the 'falcon.js' script handles the 'falconLight'/'falconDark' objects.
  // The charts should be reactive or re-initialized if the theme changes, 
  // but for the static generation, assume the current theme or a default.
  // Since Hugo static generation doesn't run JS, this code goes into the 
  // HTML/JS snippet you generate for the dashboard.
  
  // Actually, the user wants the AGENT to understand how to build these.
  // So when writing the HTML for the chart:
  var chartDom = document.getElementById('main');
  var myChart = echarts.init(chartDom, isDarkMode ? window.falconDark : window.falconLight);
  ```
...
- **Colors**: Use the `falconGreen` (#00ff9d) for primary data points.
